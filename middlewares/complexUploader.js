...
const multipleFilesUploaderMiddleware = (req, res, next) => { // next() = passe à la suite (ex : contrôleur)

	const uploader = req.app.locals.uploader; // on récupère une instance Multer qui a été configurée dans index.js

	const multipleFileUploader = uploader.array("images", 10); // crée une fonction d’upload pour un PLUSIEURS fichier (max 10)
